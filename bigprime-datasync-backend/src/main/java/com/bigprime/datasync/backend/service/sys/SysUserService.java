...
import com.bigprime.datasync.backend.constant.SuperAdminEnum;
import com.bigprime.datasync.backend.handler.sys.SysOrgHandler;
import com.bigprime.datasync.backend.handler.sys.SysUserHandler;
import com.bigprime.datasync.backend.handler.sys.SysUserRoleHandler;
import com.bigprime.datasync.backend.handler.sys.model.SysUserModel;
...
public class SysUserService {
    ...
    public List<SysUserVO> setRoleIds(List<SysUserModel> models){
        ...
        if (models.size() > 0) {
            ...
            for (SysUserModel model : models) {
                ...
                if(!StrUtil.isBlank(model.getAvatar()) && !attachmentUuids.contains(model.getAvatar())){
                    attachmentUuids.add(model.getAvatar());
                }
            }
    ...
    public UserDetail getByUserName(String name){
        SysUserModel user = modelHandler.getByUsername(name);
        if (user != null) {
            return DozerUtils.map(user, UserDetail.class);
        }
        return null;
    }
}
